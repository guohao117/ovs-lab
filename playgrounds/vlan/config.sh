...
playground_setup() {
    ...
    # Configure VLAN access ports
    for container in vlan10-c1 vlan10-c2; do
        local ofport="${CONTAINER_OFPORT[$container]}"
        echo "  Configuring $container as VLAN 10 access port (ofport $ofport)"
        ovs-vsctl set port "$container" tag=10 &>/dev/null || true
    done

    for container in vlan20-c1 vlan20-c2; do
        local ofport="${CONTAINER_OFPORT[$container]}"
        echo "  Configuring $container as VLAN 20 access port (ofport $ofport)"
        ovs-vsctl set port "$container" tag=20 &>/dev/null || true
    done

    # Configure trunk ports
    for container in trunk-c1 trunk-c2; do
        local ofport="${CONTAINER_OFPORT[$container]}"
        echo "  Configuring $container as trunk port (ofport $ofport)"
        ovs-vsctl set port "$container" trunks=10,20 &>/dev/null || true
    done
...
Example commands:
  ...
  # Monitor VLAN traffic on trunk
  ./lab.sh exec trunk-c1 "tcpdump -i eth0 -n"
